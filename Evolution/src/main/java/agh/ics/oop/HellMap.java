...
public class HellMap extends AbstractWorldMap {
    ...
    public HellMap(int width, int height, int n, int dE, boolean isEquator) { // nieczytelne nazwy: n i dE
        super(width, height, dE);
        this.isEquator = isEquator;
        if (isEquator) {  // if to nie jest najlepsze rozwiązanie
    ...
    private void findJungle() {
        ...
        for (Vector2d element : jungle) {  // czy posortowanie listy po liczbie trupów nie byłoby łatwiejsze?
            int counter = 0;
    ...
    private Vector2d randPos(Vector2d pos) { // niezrozumiałe, czemu randPos przyjmuje pos
        Vector2d toReturn = new Vector2d(randInt(0, width - 1), randInt(0, height - 1)); // do-while
    ...
    private void placeGrass() {
        ...
        while (notFound) {  // rozkład prawdopodobieństwa nie jest 80:20
            notFound = false;
            //zmienna losowa
